...
// 2^C + (-2^C & ((-2^C & X) - 1)) --> X & -2^C
static Instruction *foldAddOfPowerOf2(Instruction *I) {
  Value *And;
  const APInt *PowerOf2;

  if (!match(I, m_Add(m_Value(And), m_APInt(PowerOf2))) ||
      !PowerOf2->isPowerOf2())
    return nullptr;

  unsigned BitWidth = PowerOf2->getBitWidth();
  APInt NegPowerOf2 = -(*PowerOf2);
  Value *X;

  if (!match(And, m_And(m_Add(m_And(m_Value(X), m_SpecificInt(NegPowerOf2)),
                              m_SpecificInt(APInt::getAllOnes(BitWidth))),
                        m_SpecificInt(NegPowerOf2))))
    return nullptr;

  return BinaryOperator::CreateAnd(X,
                                   ConstantInt::get(X->getType(), NegPowerOf2));
}
...
Instruction *InstCombinerImpl::visitAdd(BinaryOperator &I) {
  ...
  if (Instruction *Res = foldAddOfPowerOf2(&I))
    return Res;
